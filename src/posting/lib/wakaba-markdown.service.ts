import { escapeHtml } from '@posting/lib/escape-html.function';
 ...
 */
@Injectable()
export class WakabaMarkdownService {
   ...
   */
  public async formatAsWakaba(input: string, boardUrl: string, isMarkdownEnabled: boolean): Promise<string> {
  ...
  private async parseCommentLinks(text: string, boardUrl: string): Promise<string> {
    ...
    while ((match = commentLinkRegex.exec(text)) !== null) {
      ...
      promises.push(
        this.commentPersistenceService.findCommentForFormatting(boardUrl, num).then((comment: Comment) => {
          if (comment) {
            if (comment.parentId) {
